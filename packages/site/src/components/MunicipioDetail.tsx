...
export interface MunicipioDetailProps {
  ...
  /** LOINC selecionado no mapa — destacado na tabela. */
  selectedLoinc: string;
  onClose: () => void;
}
      ...
      <header className="border-border flex items-start justify-between gap-2 border-b p-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 font-margem text-xs">
            Exames laboratoriais em {props.competencia} · {formatInt(totalVolume)} total ·{' '}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {rows.length === 0 ? (
          ...
          <table className="w-full font-margem text-xs">
            ...
            <tbody>
              {rows.map((r) => {
                const highlight = r.loinc === props.selectedLoinc;
                return (
                  <tr
                    key={r.loinc}
                    className={highlight ? 'bg-ps-violet/15 font-medium' : 'hover:bg-muted/40'}
                  >
