...
PORT_REGISTRY="/tmp/datasus-brasil-worktree-ports.json"
PORT_LOCKDIR="/tmp/datasus-brasil-worktree-ports.lock.d"
SITE_PORT_BASE=4322
SITE_PORT_OFFSET=10
PORT_INCREMENT=10
...
log_file() {
  echo "/tmp/datasus-brasil-site-${1//\//-}.log"
}
...
detect_branch() {
  ...
  if [[ "$dirname" == datasus-brasil-* ]]; then
...
cmd_setup() {
  ...
  echo ""
  echo "Para iniciar o dev server (quando houver site):"
...
cmd_dev() {
  ...
  if [[ ! -d "$wt_dir/site" ]]; then
    echo "Erro: nenhum diretório 'site/' em $wt_dir (site será adicionado em M4)"
    exit 1
  fi
  ...
  pnpm --filter @datasus-brasil/site dev -- --port "$site_port" 2>&1 | tee "$logfile"
}
...
cmd_list() {
  ...
  echo "Worktrees do datasus-brasil:"
  echo ""
  printf "  %-40s %-10s %-10s\n" "BRANCH" "SITE" "STATUS"
  printf "  %-40s %-10s %-10s\n" "------" "----" "------"
...
Comandos:
  setup <branch>    Cria worktree em ../datasus-brasil-<branch>, instala deps,
...
Alocação de portas:
  Worktree main sempre usa site=4322.
  Cada novo worktree recebe uma porta única começando em 4332,
  incrementando de 10 em 10.

Exemplos:
  ./scripts/worktree.sh setup feat/m1-decoder
  cd ../datasus-brasil-feat-m1-decoder
  ./scripts/worktree.sh list
  ./scripts/worktree.sh teardown feat/m1-decoder
HELP
}
